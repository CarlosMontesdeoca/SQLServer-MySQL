...
def migrateWithNews(listCodPro, idPro):
    ...
    ## ___________COMPARA SI EXISTE EL CERTIFICADO
        ...
        if(myCert):
            ...
            if(myBals):
                print(f"creating certificate {codPro}")
            else: 
                # create balance
                clsBl = data_cert[2]
                if (clsBl == 'camionera'):
            ...
            try:
                ...
                MySQLConnection.commit()
                print('  ==> SUCCESSFULLY CREATED CERTIFICATE DATA ✅')
            except:
                # logs += "==> ERROR LADING CERTIFICATE DATA \n" 
                print ("  ==> ERROR CREATED CERTIFICATE DATA ⚠")
...
## si el proyecto de MySql tiene varios certificados de SQLServer en cola  se concidera que ya recbio todos los datos y se puede migrar información    
    if len(certificates2) > 0 :
        ...
        migrateWithNews(certificates2,certificates1[0][1])
        print("========================================================================= \n\n")
